...
const generateMorosidadTemplate = (persona: PersonaMorosidadAgrupada): string => {
    let contentHTML = "";
    ...
    persona.fincas.forEach((finca) => {
        ...
        const headerRow = `
            <tr style="background-color: #d1ecf1; font-weight: bold;">
                <td colspan="4" style="padding: 8px; border: 1px solid #ddd; text-align: left;">
                    Cuenta: ${finca.servicios[0]?.codServicio || 'N/A'} | Número de Finca: ${finca.numero}
                </td>
            </tr>
        `;
        ...
        const serviceRows = finca.servicios
            .map((servicio) => {
                ...
                return `
                    <tr>
                        <td style="padding: 8px; border: 1px solid #ddd;">${servicio.nombre}</td>
                        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">${servicio.periodoDesde}</td>
                        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">${servicio.periodoHasta}</td>
                        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">${servicio.periodosAtrasados}</td>
                        <td style="padding: 8px; border: 1px solid #ddd; text-align: right;">¢${servicio.totalDeuda.toLocaleString("es-CR")}</td>
                    </tr>
                `;
            })
            .join("");
        
        // Fila Total de la Finca
        const totalFincaRow = `
            <tr style="font-weight: bold; background-color: #f2f2f2;">
                <td colspan="4" style="padding: 8px; border: 1px solid #ddd; text-align: right;">TOTAL FINCA ${finca.numero}</td>
                <td style="padding: 8px; border: 1px solid #ddd; text-align: right;">¢${totalFinca.toLocaleString("es-CR")}</td>
            </tr>
        `;
            ...
            <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 14px; border: 1px solid #ccc;">
                <thead>
                    ...
                    <tr style="background-color: #007bff; color: white;">
                        <th rowspan="2" style="padding: 8px; border: 1px solid #ddd; text-align: left;">Impuesto</th>
                        <th colspan="2" style="padding: 8px; border: 1px solid #ddd; text-align: center;">Periodo</th>
                        <th rowspan="2" style="padding: 8px; border: 1px solid #ddd; text-align: center;">Periodos Atrasados</th>
                        <th rowspan="2" style="padding: 8px; border: 1px solid #ddd; text-align: right;">Total</th>
                    </tr>
                    <tr style="background-color: #007bff; color: white;">
    ...
    const footer = `
        <div style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #ccc; font-size: 12px; color: #555;">
    ...
    return `
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 650px; margin: auto; padding: 20px; border: 1px solid #ccc; border-radius: 8px;">
                <h2 style="color: #007bff; text-align: center; border-bottom: 2px solid #007bff;">ESTADO DE CUENTA DE MOROSIDAD</h2>
                
                <div style="margin-bottom: 15px; padding: 10px; background-color: #f8f9fa; border-radius: 4px;">
                    <p style="margin: 5px 0;"><strong>Contribuyente:</strong> ${persona.nombreCompleto}</p>
                    <p style="margin: 5px 0;"><strong>Cédula:</strong> ${persona.cedula}</p>
                    <p style="margin: 5px 0;"><strong>Teléfono:</strong> ${persona.telefono}</p>
                    <p style="margin: 5px 0;"><strong>Correo:</strong> ${persona.correo}</p>
                    <p style="margin: 5px 0;"><strong>Dirección:</strong> ${persona.direccion}</p>
                </div>
                ...
                <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
                    <tr style="font-weight: bold; background-color: #f2f2f2;">
                        <td style="padding: 8px; border: 1px solid #ddd; text-align: right; width: 80%;">TOTAL GENERAL ADEUDADO:</td>
                        <td style="padding: 8px; border: 1px solid #ddd; text-align: right; color: #dc3545;">¢${persona.totalDeuda.toLocaleString("es-CR")}</td>
                    </tr>
                </table>
...
const groupDataForEmail = (listaPlana: Persona[]): GroupedData[] => {
    ...
    morosidadRecords.forEach((p) => {
        ...
        if (!fincaRecord) {
            fincaRecord = { numero: p.numeroDeFinca, servicios: [] };
            persona.fincas.push(fincaRecord);
        }

        // Servicio (Agrupado por CodServicio)
        let servicioRecord = fincaRecord.servicios.find((s) => s.codServicio === p.CodServicio);
        if (!servicioRecord) {
            servicioRecord = {
        ...
        servicioRecord.periodoDesde = Math.min(servicioRecord.periodoDesde, p.periodo);
        servicioRecord.periodoHasta = Math.max(servicioRecord.periodoHasta, p.periodo);
    ...
    morosidadMap.forEach((data) => groupedResults.push({ tipo: "Morosidad", data }));
    
    // ... (El resto de la agrupación de propiedades se mantiene igual) ...
    ...
    propiedadMap.forEach((data) => groupedResults.push({ tipo: "Propiedad", data }));
...
export const sendEmails = async (req: Request, res: Response) => {
  try {
    ...
    if (!Array.isArray(listaPlana) || listaPlana.length === 0) {
      return res.status(400).json({ error: "Debe enviar una lista de personas válida." });
    }
    ...
    const mailPromises = dataToSend.map((groupedItem) => {
      ...
      if (groupedItem.tipo === "Morosidad") {
        emailHtml = generateMorosidadTemplate(personaData as PersonaMorosidadAgrupada);
        subject = "Notificación de Estado de Cuenta de Morosidad - Municipalidad de Bagaces";
      } else {
        emailHtml = generatePropiedadTemplate(personaData as PersonaPropiedadAgrupada);
        subject = "Notificación de Fincas Omisas de Declaración - Municipalidad de Bagaces";
      }
    ...
    const morosidadCount = dataToSend.filter((d) => d.tipo === "Morosidad").length;
    const propiedadCount = dataToSend.filter((d) => d.tipo === "Propiedad").length;
    ...
    return res.status(200).json({
      ...
      personas_procesadas: dataToSend.map((p) => ({ cedula: p.data.cedula, tipo: p.tipo })),
    });
  } catch (error) {
...
const generatePropiedadTemplate = (persona: PersonaPropiedadAgrupada): string => {
  const tableRows = persona.fincas
    .map(
      ...
      <tr>
        ...
        <td style="padding: 8px; border: 1px solid #ddd;">¢${finca.valor.toLocaleString("es-CR")}</td>
      </tr>
    `
    )
    .join("");
  ...
  return `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      <div style="max-width: 600px; margin: auto; padding: 20px; border: 1px solid #eee;">
        <h3 style="border-bottom: 2px solid #ccc; padding-bottom: 10px;">
          Contribuyente: ${persona.nombreCompleto}, Cédula ${persona.cedula}
        </h3>

        <p>Estimado(a) contribuyente,</p>
        <p>La Municipalidad de Bagaces ha identificado las siguientes fincas que se encuentran a su nombre en nuestro sistema...</p>

        ${fincaTable}

        <p>Cualquier consulta a los teléfonos <strong>26901304 / 26901306 / 26901331</strong>.</p>
      </div>
    </body>
    </html>
  `;
